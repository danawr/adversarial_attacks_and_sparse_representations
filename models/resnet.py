...
def conv1x1(in_planes: int, out_planes: int, stride: int = 1) -> nn.Conv2d:
    """1x1 convolution"""
    return WS_Conv2d(in_planes, out_planes, kernel_size=1, stride=stride, bias=False)
...
class BasicBlock(nn.Module):
    ...
    def __init__(
        ...
        self.bn1 = norm_layer(8, planes)
        self.relu = nn.ReLU(inplace=True)
        self.rand1 = Randomize(planes)
        self.conv2 = conv3x3(planes, planes)
        self.bn2 = norm_layer(8, planes)
        self.rand2 = Randomize(planes)
...
class Bottleneck(nn.Module):
    ...
    def __init__(
        ...
        self.bn1 = norm_layer(8, width)
        self.conv2 = conv3x3(width, width, stride, groups, dilation)
        self.bn2 = norm_layer(8, width)
        self.conv3 = conv1x1(width, planes * self.expansion)
        self.bn3 = norm_layer(8, planes * self.expansion)
        self.relu = nn.ReLU(inplace=True)
        self.downsample = downsample
        self.stride = stride
...
class StoppableBatchNorm2d(torch.nn.BatchNorm2d):
    def __init__(self, num_features, eps=1e-5, momentum=0.1,
                 affine=True, track_running_stats=True):
        super(StoppableBatchNorm2d, self).__init__(
            num_features, eps, momentum, affine, track_running_stats)
        self.stop_updating_stats = False
    ...
    def forward(self, input):
        ...
        if self.training:
            ...
            if not self.stop_updating_stats:
...
def out_of_distribution(feature_vec, clean_stats):
    if type(clean_stats[0]) is int:
        return feature_vec
    mu, var = clean_stats
    # mu, var = torch.Tensor(mu), torch.Tensor(var)
    sigma = torch.sqrt(var)
    orig_shape = feature_vec.shape
    feature_vec = torch.flatten(feature_vec, 1)

    ood_gen_pos = torch.distributions.Normal(mu + 3 * sigma, 3 * sigma)
    random_subs_pos = torch.squeeze(ood_gen_pos.sample((feature_vec.shape[0], 1)))
    ood_gen_neg = torch.distributions.Normal(mu - 3 * sigma, 3 * sigma)
    random_subs_neg = torch.squeeze(ood_gen_neg.sample((feature_vec.shape[0], 1)))
    random_mask = torch.rand(feature_vec.shape).to('cuda')
    random_subs = torch.where(random_mask < 0.5, random_subs_pos, random_subs_neg)

    # random_subs = torch.random.normal(feature_vec.shape, mu + 3 * sigma, 3 * sigma)
    # iod_mask = torch.logical_and(feature_vec > 0, torch.abs(feature_vec - mu) < 3 * sigma)
    iod_mask = torch.abs(feature_vec - mu) < 100 * sigma
    # iod_mask = feature_vec > 0
    ood_features = torch.where(iod_mask, random_subs, feature_vec)
    random_mask = torch.rand(feature_vec.shape).to('cuda')
    flat_output_features = torch.where(random_mask < 0.5, ood_features, feature_vec)
    return torch.reshape(flat_output_features, orig_shape)
...
class ResNet(nn.Module):
    def __init__(
        ...
        self.bn1 = norm_layer(8, self.inplanes)
        self.relu = nn.ReLU(inplace=True)
        self.rand1 = Randomize(self.inplanes)
        # self.rand1 = Randomize(512)
        ...
        self.stats = {ind: (0, 0) for ind in range(0, 5)}
    ...
    def _make_layer(
        ...
        if stride != 1 or self.inplanes != planes * block.expansion:
            downsample = nn.Sequential(
                ...
                norm_layer(8, planes * block.expansion),
            )
    ...
    def get_all_features(self, x: Tensor) -> List:
        features = []
        x = self.conv1(x)
        x = self.bn1(x)
        x = self.relu(x)
        x = self.maxpool(x)
        # x = self.rand1(x)
        features.append(torch.flatten(x, 1).cpu().detach().numpy())
